...
void gpio_write_value(int gpio, int val)
{
    int fd;
    int oval;
    ...
    if (ioctl(fd, RALINK_GPIO_SET_DIR_OUT, 1 << gpio) < 0)
    {
        perror("ioctl RALINK_GPIO_SET_DIR_OUT");
        goto __OUT;
    }

    if (ioctl(fd, RALINK_GPIO_READ, &oval) < 0)
    {
        perror("ioctl RALINK_GPIO_READ");
        goto __OUT;
    }
    ...
    if (ioctl(fd, RALINK_GPIO_WRITE, oval) < 0)
    {
        perror("ioctl RALINK_GPIO_WRITE");
        goto __OUT;
    }
...
void usage(char *cmd)
{
    ...
    printf("\t\t%s e <gpio> <1|0> - set gpio to high or low level\n", cmd);
    ...
    switch (argv[1][0])
    {
        ...
        case 'e':
            if (argc != 4)
                ...
                gpio_write_value(atoi(argv[2]), atoi(argv[3]));
            break;
        default:
            usage(argv[0]);
    }
