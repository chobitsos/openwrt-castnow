...
vif VIF =
{
    ...
    .hiddenall          = {NULL, "hiddenall", NULL, NULL,  NULL},
...
param CFG_ELEMENTS[] =
{
    ...
    */
    ...
    {"HideAll", "hiddenall", NULL, hooker,  "0"},
...
void parse_uci(char * arg)
{
    ...
    uci_foreach_element(&uci_wireless->sections, e)
    {
        ...
        if(0 == strcmp(s->type, "wifi-iface"))
        {
            ...
            PARSE_UCI_OPTION(wifi_cfg[cur_dev].vifs[cur_vif].hiddenall, value);
...
void hooker(FILE * fp, param * p, const char * devname)
{
    ...
    else if(0 == strcmp(p->dat_key, "HideAll"))
    {
        for(i=0; i<wifi_cfg[N].vifnum; i++)
        {
            if (i>0) FPRINT(fp, p, ";");
            if (!wifi_cfg[N].vifs[i].hiddenall.value)
                FPRINT(fp, p, "0");
            else
                FPRINT(fp, p, wifi_cfg[N].vifs[i].hiddenall.value);
        }
    }
